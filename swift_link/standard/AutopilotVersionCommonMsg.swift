//
//  AutopilotVersionCommonMsg.swift
...
public struct AutopilotVersion {
	...
	public let capabilities: MAVProtocolCapability
...
extension AutopilotVersion: Message {
    ...
    public static var typeDescription = "Version and capability of autopilot software. This should be emitted in response to a MAV_CMD_REQUEST_AUTOPILOT_CAPABILITIES command."
    public static var fieldDefinitions: [FieldDefinition] = [("capabilities", 0, "MAVProtocolCapability", 0, "Bitmap of capabilities"), ("flightSwVersion", 16, "UInt32", 0, "Firmware version number"), ("middlewareSwVersion", 20, "UInt32", 0, "Middleware version number"), ("osSwVersion", 24, "UInt32", 0, "Operating system version number"), ("boardVersion", 28, "UInt32", 0, "HW / board version (last 8 bytes should be silicon ID, if any)"), ("flightCustomVersion", 36, "[UInt8]", 8, "Custom version field, commonly the first 8 bytes of the git hash. This is not an unique identifier, but should allow to identify the commit using the main version number even for very large code bases."), ("middlewareCustomVersion", 44, "[UInt8]", 8, "Custom version field, commonly the first 8 bytes of the git hash. This is not an unique identifier, but should allow to identify the commit using the main version number even for very large code bases."), ("osCustomVersion", 52, "[UInt8]", 8, "Custom version field, commonly the first 8 bytes of the git hash. This is not an unique identifier, but should allow to identify the commit using the main version number even for very large code bases."), ("vendorId", 32, "UInt16", 0, "ID of the board vendor"), ("productId", 34, "UInt16", 0, "ID of the product"), ("uid", 8, "UInt64", 0, "UID if provided by hardware (see uid2)")]

    public init(data: Data) throws {
		capabilities = try data.enumeration(at: 0)
